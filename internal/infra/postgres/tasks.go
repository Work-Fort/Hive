...
func (s *Store) CreateTask(ctx context.Context, t *domain.Task) error {
	...
	_, err := s.db.ExecContext(ctx,
		"INSERT INTO tasks (id, team_id, agent_id, title, description, status, flow_task_ref, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)",
...
func (s *Store) GetTask(ctx context.Context, id string) (*domain.Task, error) {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT id, team_id, agent_id, title, description, status, flow_task_ref, created_at, updated_at FROM tasks WHERE id = $1", id,
...
func (s *Store) ListTeamTasks(ctx context.Context, teamID string) ([]*domain.Task, error) {
	rows, err := s.db.QueryContext(ctx,
		"SELECT id, team_id, agent_id, title, description, status, flow_task_ref, created_at, updated_at FROM tasks WHERE team_id = $1 ORDER BY created_at",
		teamID)
	if err != nil {
...
func (s *Store) UpdateTask(ctx context.Context, id string, t *domain.Task) error {
	...
	res, err := s.db.ExecContext(ctx,
		"UPDATE tasks SET title = $1, description = $2, status = $3, agent_id = $4, flow_task_ref = $5, updated_at = NOW() WHERE id = $6",
...
func (s *Store) LookupTaskByTeamAndTitle(ctx context.Context, teamID, title string) (*domain.Task, error) {
	...
	var agentID, flowTaskRef sql.NullString
	err := s.db.QueryRowContext(ctx,
		"SELECT id, team_id, agent_id, title, description, status, flow_task_ref, created_at, updated_at FROM tasks WHERE team_id = $1 AND title = $2",
		teamID, title,
	).Scan(&t.ID, &t.TeamID, &agentID, &t.Title, &t.Description, &t.Status, &flowTaskRef, &t.CreatedAt, &t.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
